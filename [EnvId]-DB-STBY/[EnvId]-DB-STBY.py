...
Site24x7 Oracle Standby DB Plugin
...
class Oracle(object):
    def __init__(self, config):
        ...
        self.source_host = self.configurations.get('host', 'localhost')
        self.source_port = int(self.configurations.get('port', '1521'))
        self.source_sid = self.configurations.get('sid', 'XE')
        self.source_username = self.configurations.get('user', 'system')
        self.source_password = self.configurations.get('password', 'admin')
    ...
    def metricCollector(self):
        ...
        try:
            try:
                ...
                source_conn = oracledb.connect(user=self.source_username,password=self.source_password,dsn=self.source_host+':'+str(self.source_port)+'/'+self.source_sid)
            ...
            if SYNC_STATUS:
                ...
                for row in c:
                    self.data['STBY_applying_sequence'] = row[4]
                ...
                for row in c:
                    if row[0] == 1:
                        self.data['STBY_running_sync_jobs'] = 1
                    else:
                        self.data['STBY_running_sync_jobs'] = 0

                c.execute("SELECT COUNT(*) FROM GV$MANAGED_STANDBY where STATUS='ERROR'")
                for row in c:
                    if row[0] == 0:
                        self.data['STBY_failed_sync_jobs'] = 0
                    else:
                        self.data['STBY_failed_sync_jobs'] = 1
...
if __name__ == "__main__":
    configurations = {'host': ORACLE_HOST, 'port': ORACLE_PORT,
					  ...
					  'source_host': SOURCE_DB_HOST, 'source_port': SOURCE_DB_PORT, 
					  'source_sid': SOURCE_DB_SID, 'source_username': SOURCE_DB_USERNAME, 'source_password': SOURCE_DB_PASSWORD}
